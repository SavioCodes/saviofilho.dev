...
<SystemDiagram
  ...
  footer="O sinal de produto vem de como esses estados se conectam, não da interface do formulário."
>
  <FlowStep
    step="01"
    title="Criar a fronteira do tenant"
    detail="O signup cria um espaço escopado por organização, com trial e ownership definidos para tudo o que vem depois."
  />
  <FlowStep
    step="02"
    title="Passar upgrade pelo billing"
    detail="Preferências do Mercado Pago e replay seguro de webhook decidem quando a assinatura pode ficar ativa sem duplicar estado."
  />
  <FlowStep
    step="03"
    title="Mover documentos em storage controlado"
    detail="Uploads funcionam no ambiente local e depois trocam de forma limpa para storage compatível com S3 quando o deploy fica real."
  />
  <FlowStep
    step="04"
    title="Gerar follow-up com cheque de custo"
    detail="Os follow-ups via cron só avançam quando teto de uso e estado da conta continuam dentro do que o produto permite."
  />
</SystemDiagram>
