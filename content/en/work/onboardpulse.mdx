...
<SystemDiagram
  title="Operating route"
  subtitle="Tenant boundary, billing lifecycle, storage, and follow-up automation"
  footer="The product signal comes from how these states connect, not from the form UI."
>
  <FlowStep
    step="01"
    title="Create the tenant boundary"
    detail="Signup creates an organization-scoped space, trial state, and account ownership that everything else must respect."
  />
  <FlowStep
    step="02"
    title="Route upgrades through billing"
    detail="Mercado Pago preferences and webhook replay handling decide whether the subscription becomes active without duplicating state."
  />
  <FlowStep
    step="03"
    title="Move documents through controlled storage"
    detail="Uploads work locally in development and switch cleanly to S3-compatible storage when the deployment surface becomes real."
  />
  <FlowStep
    step="04"
    title="Generate follow-ups with budget checks"
    detail="Cron-driven follow-ups only happen after usage ceilings and account state are still within acceptable bounds."
  />
</SystemDiagram>
