...
<SystemDiagram
  title="API trust route"
  subtitle="Contract first, then auth, then classification, then deploy verification"
  footer="The interesting part is not that the API responds. It is that another engineer can trust the product surface quickly."
>
  <FlowStep
    step="01"
    title="Accept a narrow contract"
    detail="The API starts from a clear OpenAPI schema so request shape, auth, and response fields are readable before any code inspection."
  />
  <FlowStep
    step="02"
    title="Gate traffic at the edge"
    detail="API keys and rate limiting run before deeper classification so misuse is constrained before the business logic is even reached."
  />
  <FlowStep
    step="03"
    title="Score with layered signals"
    detail="Disposable domains, typos, and optional external lookups produce a response that stays small but still feels product-ready."
  />
  <FlowStep
    step="04"
    title="Verify the deploy surface"
    detail="Docker Compose, healthchecks, and smoke scripts make the repo feel like something another engineer could publish without guesswork."
  />
</SystemDiagram>
